...
async function setBotCommands(bot: Bot<BotContext>) {
	...
	const publicCommands = [
		...
		{ command: "community", description: "Node route stats 节点路由统计" },
	...
	if (config.adminChatId) {
		...
		await bot.api.setMyCommands([...publicCommands, ...adminCommands], {
...
async function notifyPendingOnStartup(bot: Bot<BotContext>) {
	...
	try {
		...
		let message =
			`🔔 *Startup: ${sessions.length} pending request(s)*\n` +
		...
		console.log(
			`[Startup] Notified admin about ${sessions.length} pending request(s)`,
		);
	} catch (error) {
...
function startMigrationNotifyChecker(bot: Bot<BotContext>) {
	...
	const intervalId = setInterval(async () => {
		try {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const targets = ((targetsResult.data as any)?.targets || []) as Array<{
				asn: number;
				telegramId?: number;
				emails?: string[];
			}>;
			...
			for (const item of ready) {
				...
				if (target?.telegramId) {
					...
					try {
						await bot.api.sendMessage(target.telegramId, message, {
							parse_mode: "Markdown",
						});
						delivered = "tg";
					} catch (e) {
			...
			if (ackAsns.length > 0) {
				await apiRequest(
					"/admin",
					"POST",
					{ action: "ackMigrationNotify", asns: ackAsns },
					config.apiToken,
				);
			}
			console.log(
				`[MigrateNotify] terminal=${terminal.length} (acked ${ackAsns.length}), retrying=${retrying}`,
			);
			...
			if (adminChatId && terminal.length > 0) {
				...
				await bot.api.sendMessage(adminChatId, summary, {
					parse_mode: "Markdown",
				});
			}
		} catch (error) {
	...
	console.log(
		`[MigrateNotify] Checker started (interval: ${CHECK_INTERVAL / 1000}s)`,
	);
