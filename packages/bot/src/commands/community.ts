...
export function registerCommunityCommands(bot: Bot<BotContext>) {
	...
	bot.command("community", async (ctx) => {
		...
		const loading = await ctx.reply(
			`📊 Fetching community stats from ${nodeName}...`,
		);

		try {
			const stats = (await callAgentApi(
				nodeId,
				"GET",
				"/community",
			)) as CommunityStats | null;
			...
			const text = COMMUNITY_STATS.replace("{node}", escapeMarkdown(nodeName))
	...
	bot.callbackQuery(/^community:(.+)$/, async (ctx) => {
		...
		if (!stats) {
			...
			await ctx.editMessageText(
				`❌ Failed to load community stats from ${nodeName}.\n无法从该节点获取统计。`,
			);
			return;
		}
	...
	bot.callbackQuery(/^probe_now:([^:]+):(\d+)$/, async (ctx) => {
		...
		const result = (await callAgentApi(node, "POST", "/probe/now", {
			asn,
		})) as ProbeResult | null;

		if (result?.success) {
			await ctx.answerCallbackQuery(
				`✅ ${result.rtt_ms?.toFixed(1)}ms (Tier ${result.latency_tier})`,
			);
			// Edit the existing latency message in place instead of sending a new
			// one on every probe (was flooding the chat).
			await showLatencyStats(
				ctx,
				asn,
				node,
			...
			await ctx.answerCallbackQuery(
				`❌ Probe failed: ${result?.error || "Unknown error"}`,
			);
		}
	});
}
 ...
 */
export async function showLatencyStats(
	ctx: BotContext,
	asn: number,
	node: string,
	editId?: number,
	...
	const stats = (await callAgentApi(node, "POST", "/probe/stats", {
		asn,
	})) as ProbeStats | null;
	...
	let text: string;
	if (stats?.last_rtt) {
		text = LATENCY_STATS.replace("{asn}", String(asn))
