...
export async function submitModifyChanges(flow: {
	...
	if (
		current.mpbgp !== backup?.mpbgp ||
		current.extendedNexthop !== backup?.extendedNexthop
	) {
		requestBody.extensions =
			(current.mpbgp ? "mp_bgp" : "") +
			(current.extendedNexthop ? ",extended_nexthop" : "");
	}
	...
	if (hasFieldChanges) {
		console.log(
			"[submitModifyChanges] Request body:",
	...
	if (flow.pendingMigration) {
		const migrateResult = await apiRequest(
			"/admin",
			"POST",
			{
				action: "migrate",
		...
		if (migrateResult.code !== 0) {
			return {
				success: false,
				migrated: false,
				message: `Migration failed: ${migrateResult.message}`,
			};
		}
