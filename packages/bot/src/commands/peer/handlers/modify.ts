	...
	switch (field) {
		case "endpoint": {
			if (text.toLowerCase() === "none" || /nat/i.test(text)) {
				...
				if (port && !/^\d+$/.test(port))
					return "Invalid port. Use `host:port` or `none`.";
		...
		case "pubkey": {
			if (!isValidWgPubkey(text))
				return "Invalid WireGuard public key (44 chars ending with `=`).";
		...
		case "contact": {
			if (text.length < 3 || text.length > 200)
				return "Contact must be 3–200 characters.";
...
const FIELD_PROMPTS: Record<TextField, string> = {
	endpoint:
		"📡 Send the new endpoint as `host:port`, or tap None (NAT).\n输入新端点 `host:端口`，或点 None。",
	pubkey:
		"🔑 Send the new WireGuard public key (44 chars ending `=`).\n输入新的 WireGuard 公钥。",
	contact: "📇 Send the new contact info (3–200 chars).\n输入新的联系方式。",
	peerIpv6: "🌐 Send the peer IPv6 (fe80:/fd../fc..).\n输入对方 IPv6。",
	peerIpv4:
		"🌐 Send the peer IPv4 (172.16–23.x.x), or tap None.\n输入对方 IPv4，或点 None。",
	localIpv6: "🌐 Send the local IPv6 (fe80:/fd../fc..).\n输入我方 IPv6。",
	localIpv4:
		"🌐 Send the local IPv4 (172.16–23.x.x), or tap None.\n输入我方 IPv4，或点 None。",
};

/**
 * Show modify menu helper type - will be passed from peer.ts
 */
type ShowModifyMenuFn = (
	ctx: BotContext,
	isFirstTime?: boolean,
) => Promise<void>;
...
export function registerModifyHandlers(
	...
	bot.callbackQuery("modify:submit", async (ctx) => {
		...
		try {
			...
			await ctx.reply(
				`❌ Failed to submit changes: ${error instanceof Error ? error.message : "Unknown error"}`,
				{ reply_markup: { remove_keyboard: true } },
			);
		}
	...
	bot.callbackQuery("modify:m:stype", async (ctx) => {
		...
		const kb = new InlineKeyboard()
			...
			.row()
			.text("IPv6 + IPv4 (独立)", "modify:sessionType:separate")
			.row()
	...
	bot.callbackQuery("modify:m:psk", async (ctx) => {
		...
		const kb = new InlineKeyboard();
		if (flow?.current?.psk) {
			kb.text("🔄 Regenerate", `modify:psk:${uuid}:generate`)
				.row()
				.text("❌ Disable", `modify:psk:${uuid}:disable`)
				.row();
		} else {
	...
	bot.callbackQuery("modify:m:mtu", async (ctx) => {
		...
		presets.forEach((m, i) => {
			kb.text(
				m === 1420 ? "1420 (默认)" : String(m),
				`modify:mtu:${uuid}:${m}`,
			);
			if (i % 3 === 2) kb.row();
		});
		kb.row().text("🔙 Back", "modify:m:menu");
		await ctx.editMessageText("📏 *MTU* — pick a value:", {
			parse_mode: "Markdown",
			reply_markup: kb,
		});
	});
	...
	bot.callbackQuery("modify:m:bgp", async (ctx) => {
		...
		const kb = new InlineKeyboard()
			.text("Peer IPv6 对方", "modify:m:peerIpv6")
			.text("Peer IPv4 对方", "modify:m:peerIpv4")
			.row()
			.text("Local IPv6 我方", "modify:m:localIpv6")
			.text("Local IPv4 我方", "modify:m:localIpv4")
			.row()
			.text("🔙 Back", "modify:m:menu");
		await ctx.editMessageText(
			`🌐 *BGP Address*\n\n` +
	...
	const promptField = async (ctx: BotContext, field: TextField) => {
		...
		kb.text("🔙 Back", "modify:m:menu");
		await ctx.editMessageText(FIELD_PROMPTS[field], {
			parse_mode: "Markdown",
			reply_markup: kb,
		});
	};
	for (const f of [
		...
		bot.callbackQuery(`modify:m:${f}`, async (ctx) => {
			await promptField(ctx, f);
		});
	}

	// "None" shortcut for endpoint / ipv4 fields.
	bot.callbackQuery(
		/^modify:m:none:(endpoint|peerIpv4|localIpv4)$/,
		async (ctx) => {
