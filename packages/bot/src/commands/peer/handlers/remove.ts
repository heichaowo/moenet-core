...
export function registerRemoveHandlers(bot: Bot<BotContext>) {
	...
	bot.callbackQuery(/^remove:select:(.+)$/, async (ctx) => {
		...
		const ownerCheck = await apiRequest(
			"/admin",
			"POST",
			{ action: "getSession", uuid },
			config.apiToken,
		);
		const ownerSession = ownerCheck.data?.session as
			| { asn?: number }
			| undefined;
		...
		const removeCode = Array.from(bytes)
	...
	bot.callbackQuery(/^remove:confirm:(.+)$/, async (ctx) => {
		...
		if (
			ctx.session.peerFlow?.sessionUuid !== uuid ||
