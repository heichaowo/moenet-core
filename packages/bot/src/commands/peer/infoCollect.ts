 ...
 */
export async function preRegion(
	ctx: BotContext,
	peerInfo: PeerInfo,
	...
	try {
		...
		if (result.code !== 0 || !result.data?.routers) {
			await ctx.reply("❌ 当前没有可用节点 / No available nodes", {
...
export async function postRegion(
	...
	if (!peerInfo.nodeMap || !peerInfo.nodeMap[text]) {
		...
		const keyboard: { text: string }[][] = labels.map((label) => [
			{ text: label },
		]);

		await ctx.reply("❌ 无效选择，请重试 / Invalid selection, try again:", {
			reply_markup: {
				keyboard,
				resize_keyboard: true,
			},
 ...
 */
export async function preSessionType(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	await ctx.reply(
...
export async function postSessionType(
	...
	if (text.includes("ipv6") && text.includes("ipv4")) {
		...
		await ctx.reply("❌ 无效输入，请重试 / Invalid input, try again:", {
			reply_markup: {
				keyboard: [
					[
						{ text: "IPv6 & IPv4" },
 ...
 */
export async function preMpbgp(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	await ctx.reply("Do you support Multi-Protocol BGP?\n你支持多协议 BGP 吗？", {
		reply_markup: {
 ...
 */
export async function postMpbgp(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	const text = (ctx.message?.text || "").trim().toLowerCase();
	...
	if (text === "yes") {
		...
		await ctx.reply("❌ 无效输入，请输入 Yes 或 No:", {
			reply_markup: {
				keyboard: [[{ text: "Yes" }, { text: "No" }]],
				resize_keyboard: true,
			},
 ...
 */
export async function preMpbgpType(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	await ctx.reply(
 ...
 */
export async function postMpbgpType(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	const text = (ctx.message?.text || "").trim().toLowerCase();
	...
	if (text === "ipv6") {
		...
		await ctx.reply("❌ 无效输入，请选择 IPv6 或 IPv4:", {
			reply_markup: {
				keyboard: [[{ text: "IPv6" }, { text: "IPv4" }]],
				resize_keyboard: true,
			},
 ...
 */
export async function preEnh(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	await ctx.reply("Do you support Extended Next Hop?\n你支持扩展的下一跳吗？", {
		reply_markup: {
			keyboard: [[{ text: "Yes" }, { text: "No" }]],
			resize_keyboard: true,
			one_time_keyboard: true,
...
export async function preIpv6(
	...
	if (
		peerInfo.IPv6 &&
		!["Not enabled", "Not required"].includes(peerInfo.IPv6)
	) {
	...
	const keyboard: { text: string }[][] = suggestions
		.slice(0, 3)
		.map((s) => [{ text: s }]);
 ...
 */
export async function postIpv6(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
 ...
 */
export async function preRequestLinklocal(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
 ...
 */
export async function postRequestLinklocal(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	let text = (ctx.message?.text || "").trim();
 ...
 */
export async function preLocalIpv6(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	...
	let suggested = "";
	if (
		peerIpv6.toLowerCase().startsWith("fc") ||
		peerIpv6.toLowerCase().startsWith("fd")
	) {
 ...
 */
export async function postLocalIpv6(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	let text = (ctx.message?.text || "").trim();
...
export async function preIpv4(
	...
	if (
		peerInfo.IPv4 &&
		!["Not enabled", "Not required (ENH)"].includes(peerInfo.IPv4)
	) {
 ...
 */
export async function postIpv4(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	let text = (ctx.message?.text || "").trim();
 ...
 */
export async function preLocalIpv4(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	// Suggest adjacent IP
 ...
 */
export async function postLocalIpv4(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	let text = (ctx.message?.text || "").trim();
	...
	if (!isValidDN42IPv4(text)) {
		await ctx.reply("❌ 无效的 DN42 IPv4 地址\nInvalid DN42 IPv4 address", {
			reply_markup: { remove_keyboard: true },
		});
		return { nextStep: "post_local_ipv4", peerInfo };
	}
 ...
 */
export async function preClearnet(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	const keyboard: { text: string }[][] = [];
 ...
 */
export async function postClearnet(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	let text = (ctx.message?.text || "").trim();
 ...
 */
export async function postPort(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	...
	if (!isValidPort(port)) {
		await ctx.reply("❌ 无效端口，请输入 1-65535 之间的数字", {
			reply_markup: { remove_keyboard: true },
		});
		return { nextStep: "post_port", peerInfo };
	}
 ...
 */
export async function prePubkey(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	const keyboard: { text: string }[][] = [];
 ...
 */
export async function postPubkey(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult | null> {
	const text = (ctx.message?.text || "").trim();
...
export async function prePsk(
	...
	await ctx.reply(
			...
			"• 或直接输入你的 PSK (44字符 base64)",
		{
			parse_mode: "Markdown",
			reply_markup: {
				keyboard: [[{ text: "🔄 Auto Generate" }, { text: "❌ No PSK" }]],
				resize_keyboard: true,
				one_time_keyboard: true,
			},
		},
...
export async function postPsk(
	...
	if (text.length === 44 && text.endsWith("=")) {
		peerInfo.PSK = text;
		await ctx.reply(`🔑 PSK 已设置 / PSK Set:\n\`${text}\``, {
			parse_mode: "Markdown",
			reply_markup: { remove_keyboard: true },
		});
		return await preContact(ctx, peerInfo);
 ...
 */
export async function preContact(
	ctx: BotContext,
	peerInfo: PeerInfo,
): Promise<StepResult> {
	const suggestions: string[] = [];
 ...
 */
export const STEP_FUNCTIONS: Record<
	string,
	(ctx: BotContext, peerInfo: PeerInfo) => Promise<StepResult | null>
> = {
