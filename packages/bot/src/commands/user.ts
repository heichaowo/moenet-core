 ...
 */
async function registerUserTelegramId(
	asn: number,
	telegramId: number,
): Promise<void> {
	try {
...
function extractFingerprintFromGpgOutput(stderr: string): string {
	...
	for (const line of stderr.split("\n")) {
		if (line.includes("Primary key fingerprint:")) {
			...
			line.toLowerCase().includes("fingerprint:") &&
			!primaryFingerprint
		) {
			const parts = line.split(":");
			const lastPart = parts[parts.length - 1];
 ...
 */
async function gpgDecryptChallenge(
	sigPath: string,
): Promise<{ content: string | null; stderr: string }> {
	try {
 ...
 */
async function recvGpgKeyFromKeyserver(
	fingerprint: string,
	keyserver: string,
): Promise<boolean> {
	try {
...
async function tryGpgVerifyFingerprint(
	...
	try {
		...
		if (!signatureFingerprint) {
			return {
				success: false,
				error: "Could not extract fingerprint from signature",
			};
		}

		const fingerprintsUpper = gpgFingerprints.map((fp) =>
			fp.replace(/ /g, "").toUpperCase(),
		);

		const fingerprintMatched = fingerprintsUpper.some(
			(fp) =>
...
async function verifySshSignatureFull(
	...
	try {
		...
		const combined = result.stdout + result.stderr;
		const verified =
			result.code === 0 || /Good\s+"?\w+"?\s+signature/i.test(combined);

		if (!verified) {
			const errMsg =
				result.stderr.trim() || result.stdout.trim() || "Verification failed";
...
function normalizeSshSignature(raw: string): string {
	...
	sanitized = sanitized
		// biome: noMisleadingCharacterClass \u2014 ZWJ is a combining char; use alternation
		.replace(/\u200B|\u200C|\u200D|\uFEFF/g, "");
	...
	return [SSH_SIG_HEADER, ...wrappedLines, SSH_SIG_FOOTER, ""].join("\n");
}
...
export function registerUserCommands(bot: Bot<BotContext>) {
	...
	bot.command("cancel", async (ctx) => {
		...
		if (ctx.session.modifyInput) {
			ctx.session.modifyInput = undefined;
			hadFlow = true;
		}
		if (ctx.session.nodeWizard) {
			ctx.session.nodeWizard = undefined;
			hadFlow = true;
		}
		if (ctx.session.nodeEdit) {
	...
	bot.on("message:text", async (ctx, next) => {
		...
		if (!isAsnInput(text)) {
			await ctx.reply(
				`${ICONS.error} Invalid ASN format. Example: 998, 0998, AS4242420998\n无效的 ASN 格式。`,
			);
			return;
		}
		...
		if (asn < 4242420000 || asn > 4242429999) {
			await ctx.reply(
				`${ICONS.error} Invalid ASN. DN42 range: 4242420000-4242429999\n无效的 ASN。DN42 范围: 4242420000-4242429999`,
			);
			return;
		}
		...
		try {
			...
			if (result.code !== 0) {
				await ctx.reply(
					`${ICONS.error} Error: ${result.message ?? "Unknown error"}`,
				);
				return;
			}
			...
			for (const method of availableAuthMethods) {
				...
				const val =
					method.value || method.name || method.data || method.fingerprint;
			...
			console.log(
				`[Login] AS${asn} auth methods - GPG: ${gpgFingerprints.length}, SSH: ${sshKeys.length}, Email: ${emails.length}`,
			);
			...
			await ctx.reply(
				`${ICONS.error} Failed to query authentication methods\n查询认证方式失败`,
			);
		}
	});
	...
	bot.callbackQuery(/^login:gpg:(\d+)$/, async (ctx) => {
		...
		const fpDisplay =
			gpgFp.length > 16 ? `${gpgFp.slice(0, 8)}...${gpgFp.slice(-8)}` : gpgFp;
	...
	bot.callbackQuery(/^login:ssh:(\d+)$/, async (ctx) => {
		...
		const sshKeyDisplay =
			sshKey.length > 60 ? `\`${sshKey.slice(0, 60)}...\`` : `\`${sshKey}\``;
		...
		try {
			if (method === "email") {
				...
				if (text === challenge) {
					...
					registerUserTelegramId(asn, userId)
						.then(() => {
					...
					await ctx.reply(
						`${ICONS.error} Invalid code. Try again.\n验证码错误，请重试。`,
					);
				}
			} else if (method === "gpg") {
			...
			await ctx.reply(
				`${ICONS.error} Verification error: ${(error as Error).message}`,
			);
		}
	});
	...
	bot.command("logout", async (ctx) => {
		...
		await ctx.reply(
			`${ICONS.logout} Logged out from AS${asn}\n已退出 AS${asn}`,
		);
	});
