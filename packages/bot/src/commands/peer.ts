...
import {
	CN_REJECTION_MESSAGE,
	isChinaIP,
	resolveEndpoint,
} from "../providers/chinaIp";
import {
	isDN42IPv4,
	isDN42ULA,
	isLinkLocal,
	validateIpOwnership,
} from "../services/dn42Validator";
import { DIVIDER } from "../templates";
import { showLatencyStats } from "./community";
import { endpointSyncIssue, evaluatePeerRequest } from "./peer/approvalCard";
...
async function showModifyMenu(ctx: BotContext, isFirstTime = false) {
	...
	const mpbgpText = current.mpbgp
		? current.extendedNexthop
			? "IPv6 (ENH)"
			: "IPv6"
		: "Not supported";
...
async function showPeerList(
	...
	if (sessions.length === 0) {
		...
		for (const s of sessions) {
			...
			kb.text(
				`${peerDot(s.status)} ${s.routerName || s.router}`,
				`peer:v:${s.uuid}`,
			).row();
		}
	}
	...
	if (editId) {
		try {
			await ctx.api.editMessageText(ctx.chat!.id, editId, text, {
				parse_mode: "Markdown",
				reply_markup: kb,
			});
		} catch (e) {
...
async function showPeerDetail(ctx: BotContext, uuid: string, editId?: number) {
	...
	if (result.code !== 0 || !s) {
		await ctx.reply("❌ Peer not found.\n找不到该 Peer。");
		return;
	}
	if (!isAdmin(ctx) && Number(s.asn) !== ctx.session.asn) {
		const msg = "❌ This peer does not belong to you.\n这不是你的 Peer。";
		if (editId) await ctx.api.editMessageText(ctx.chat!.id, editId, msg);
	...
	const channel =
		extStr.includes("mp_bgp") || extStr.includes("mpbgp")
			? "IPv6 & IPv4"
			: "IPv6 only";
	...
	const render = (pm?: "Markdown") =>
		editId
			? ctx.api.editMessageText(ctx.chat!.id, editId, text, {
					parse_mode: pm,
					reply_markup: kb,
				})
...
async function showAdminPeerPanel(ctx: BotContext, editId?: number) {
	...
	const kb = new InlineKeyboard()
		.text("📋 All sessions 所有会话", "peer:all")
		.row()
		.text("➕ Add peer 添加", "peer:adminadd")
		.row()
		.text("⏳ Pending 待审核", "admin:pending")
		.row()
		.text("🔍 By ASN 按 ASN 查", "peer:byasn")
		.row()
...
async function showAllSessions(ctx: BotContext, editId?: number, page = 0) {
	...
	sessions.sort(
		(a, b) =>
			a.asn - b.asn ||
			(a.routerName || a.router).localeCompare(b.routerName || b.router),
	);

	const asnCount = new Set(sessions.map((s) => s.asn)).size;
	const totalPages = Math.max(
		1,
		Math.ceil(sessions.length / ALL_SESSIONS_PAGE),
	);
	const p = Math.min(Math.max(page, 0), totalPages - 1);
	const pageItems = sessions.slice(
		p * ALL_SESSIONS_PAGE,
		p * ALL_SESSIONS_PAGE + ALL_SESSIONS_PAGE,
	);
...
export function registerPeerCommands(bot: Bot<BotContext>) {
	...
	bot.command("peer", async (ctx) => {
		...
		if (args[0] && isAsnInput(args[0])) {
			if (!isAdmin(ctx)) {
				await ctx.reply(
					"❌ Only admin can view other ASN peers\n只有管理员可查看其他 ASN 的 Peer",
				);
				return;
			}
	...
	async function startPeerCreation(
		ctx: BotContext,
		opts?: { adminMode?: boolean; targetAsn?: number },
	) {
		...
		try {
			...
			const keyboard = new InlineKeyboard();
			for (let i = 0; i < couldPeer.length; i++) {
				// biome: useIterableCallbackReturn — forEach callback must not return a value
				keyboard.text(couldPeer[i] ?? "", `peer:pick:${i}`).row();
			}
	...
	bot.callbackQuery("peer:list", async (ctx) => {
		if (!ctx.session.asn) {
			await ctx.answerCallbackQuery("❌ /login first");
			return;
		}
	...
	bot.callbackQuery("peer:all:noop", async (ctx) => {
		await ctx.answerCallbackQuery();
	});
	...
	bot.callbackQuery("peer:byasn", async (ctx) => {
		...
		await ctx.answerCallbackQuery();
		await ctx.reply(
			"🔍 Enter ASN to view (e.g. `998` / `AS4242420998`):\n输入要查看的 ASN:",
			{ parse_mode: "Markdown" },
		);
	});
	bot.callbackQuery("peer:adminadd", async (ctx) => {
		...
		await ctx.answerCallbackQuery();
		await ctx.reply(
			"➕ Enter ASN to add a peer for (e.g. `4242420998`):\n输入要为其添加 Peer 的 ASN:",
			{ parse_mode: "Markdown" },
		);
	});
	...
	bot.on("message:text", async (ctx, next) => {
		...
		if (Number.isNaN(asn)) {
			await ctx.reply("❌ Invalid ASN. 无效 ASN。");
			return;
		}
	...
	bot.callbackQuery(/^peer:st:(.+)$/, async (ctx) => {
		...
		const r = await apiRequest(
			"/admin",
			"POST",
			{ action: "getSession", uuid: ctx.match[1] },
			config.apiToken,
		);
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
	...
	bot.callbackQuery(/^peer:pick:(\d+)$/, async (ctx) => {
		...
		if (
			!flow ||
			flow.step !== "select_node" ||
			!flow.nodeMap ||
			!flow.couldPeerLabels
		) {
	...
	bot.callbackQuery(/^peer:here:(.+)$/, async (ctx) => {
		...
		try {
			...
			if (!r.isOpen || !hasCapacity) {
				await ctx.reply(
					`❌ \`${name}\` is not open for peering right now.\n该节点当前不可 Peer。`,
					{ parse_mode: "Markdown" },
				);
				return;
			}
	...
	bot.on("message:text", async (ctx, next) => {
		...
		switch (ctx.session.peerFlow?.step || flow.step) {
			...
			case "select_node": {
				...
				if (!nodeMap) {
					await ctx.reply("❌ Error: Node map not found", {
				...
				if (!nodeInfo) {
					await ctx.reply(
						"❌ Invalid node. Please select from the list.\n无效节点，请从列表中选择。",
						{ reply_markup: { remove_keyboard: true } },
					);
					return;
				}
			...
			case "select_session_type": {
				...
				if (text.includes("MP-BGP") || text.includes("ENH")) {
					ctx.session.peerFlow = {
						...flow,
						step: "input_ipv6",
						sessionType: "ipv6_only",
					};
					// Use targetAsn for admin mode, session.asn for user mode
					const asn = flow.isAdminMode
						? flow.targetAsn || 0
						: ctx.session.asn || 0;
					const suggested = `fe80::${asn % 10000}`;
					await ctx.reply(`✅ Session Type: *MP-BGP + ENH*`, {
						parse_mode: "Markdown",
					});
				...
				if (text.includes("ULA") || text.includes("GUA")) {
					ctx.session.peerFlow = {
						...flow,
						step: "input_peer_ipv6_ula",
						sessionType: "ipv6_ipv4",
					};
					await ctx.reply(`✅ Session Type: *ULA/GUA Mode*`, {
						parse_mode: "Markdown",
					});
			...
			case "input_mtu": {
				...
				if (!mtu) {
					...
					if (isNaN(parsed) || parsed < 1280 || parsed > 1500) {
						await ctx.reply(
							"❌ Invalid MTU. Please enter 1280-1500.\n无效的 MTU，请输入 1280-1500",
						);
						return;
					}
					mtu = parsed;
			...
			case "input_contact": {
				...
				await ctx.reply(`✅ Contact: \`${selectedContact}\``, {
					parse_mode: "Markdown",
					reply_markup: { remove_keyboard: true },
				});
				await showConfirmation(ctx);
			...
			case "input_contact_manual": {
				...
				if (manualContact.length < 3 || manualContact.length > 200) {
					await ctx.reply(
						"❌ Contact must be 3-200 characters.\n联系方式长度须为 3-200 个字符。",
					);
					return;
				}
				ctx.session.peerFlow = {
				...
				await ctx.reply(`✅ Contact: \`${manualContact}\``, {
					parse_mode: "Markdown",
					reply_markup: { remove_keyboard: true },
				});
				await showConfirmation(ctx);
			...
			case "input_ipv6": {
				...
				if (!isValidIPv6(ipv6 || "")) {
					await ctx.reply(
						"❌ Invalid IPv6 address. Please try again.\n无效的 IPv6 地址，请重试。",
					);
					return;
				}
			...
			case "input_endpoint": {
				...
				ctx.session.peerFlow = {
					...flow,
					step: port ? "input_pubkey" : "input_port",
					endpoint,
					port,
				};

				if (port) {
					await ctx.reply(`✅ Endpoint: \`${endpoint}:${port}\``, {
						parse_mode: "Markdown",
					});
			...
			case "input_pubkey": {
				if (!isValidWgPubkey(text)) {
					await ctx.reply(
						"❌ Invalid WireGuard public key. Should be 44 characters ending with =",
					);
					return;
				}
				...
				if (text.toLowerCase() === "yes") {
					...
					try {
						...
						let evaluation: Awaited<
							ReturnType<typeof evaluatePeerRequest>
						> | null = null;
						...
						if (!flow.isAdminMode && config.adminChatId && evaluation) {
							...
							for (let attempt = 1; attempt <= 3; attempt++) {
								try {
									...
									console.error(
										`[Notify Admin] Attempt ${attempt}/3 failed:`,
										e,
									);
									if (attempt < 3) {
			...
			case "remove_confirm": {
				...
				if (!expectedCode) {
					ctx.session.peerFlow = undefined;
					await ctx.reply(
						"❌ Error: No confirmation code. Please retry /remove",
					);
					return;
				}
	...
	bot.command("info", async (ctx) => {
		...
		if (args[0] && isAsnInput(args[0])) {
			if (!isAdmin) {
				await ctx.reply(
					"❌ Only admin can view other ASN info\n只有管理员可以查看其他 ASN 的信息",
				);
				return;
			}
	...
	async function fetchAndDisplayInfo(
		...
		try {
			...
			const result = await apiRequest(
				"/admin",
				"POST",
				{ action: "enumSessions", asn: targetAsn },
				config.apiToken,
			);
			...
			const activeSessions = sessions.filter(
				(s) => s.status === PeeringStatus.ENABLED,
			);
			...
			for (const [i, s] of sessions.entries()) {
				const statusIcon =
					s.status === PeeringStatus.ENABLED
						? "🟢"
						: s.status === PeeringStatus.PENDING_REVIEW
							? "⏳"
							: "❌";
				const statusText =
					s.status === PeeringStatus.ENABLED
				...
				if (s.endpoint)
					message += `   ${useAdminApi ? "Peer" : "Your"} Endpoint: \`${s.endpoint}\`\n`;
				if (s.serverEndpoint)
					message += `   🖥️ Server Endpoint: \`${s.serverEndpoint}\`\n`;
				if (s.serverWgKey)
					message += `   🔑 Server Key: \`${s.serverWgKey.slice(0, 10)}...\`\n`;
				...
				if (s.status === PeeringStatus.ENABLED) {
					...
					if (live) {
						...
						const routeInfo =
							live.routes_imported !== undefined &&
							live.routes_exported !== undefined
	...
	async function presentModifyPeerPicker(ctx: BotContext, targetAsn: number) {
		try {
			...
			if (sessions.length === 0) {
				await ctx.reply(
					`ℹ️ AS${targetAsn} has no peers to modify.\nAS${targetAsn} 没有可修改的 Peer`,
				);
				return;
			}
	...
	bot.command("modify", async (ctx) => {
		...
		if (args[0] && isAsnInput(args[0])) {
			if (!isAdmin) {
				await ctx.reply(
					"❌ Only admin can modify other ASN peers\n只有管理员可以修改其他 ASN 的 Peer",
				);
				return;
			}
		...
		try {
			...
			if (result.code !== 0) {
				await ctx.editMessageText(
					`❌ Failed to fetch session: ${result.message}`,
				);
				return;
			}
			...
			if (!isAdmin(ctx) && Number(session.asn) !== ctx.session.asn) {
				await ctx.editMessageText(
					"❌ This peer does not belong to you.\n这不是你的 Peer。",
				);
				return;
			}
			...
			const hasEnh =
				extStr.includes("extended_nexthop") || extStr.includes("enh");
	...
	bot.callbackQuery(/^modify:field:(.+):(.+)$/, async (ctx) => {
		...
		switch (field) {
			case "region":
				...
				try {
					const nodeResult = await apiRequest(
						"/admin",
						"POST",
						{ action: "enumRouters" },
						config.apiToken,
					...
					if (nodeResult.code === 0 && Array.isArray(nodes)) {
						...
						for (const node of nodes) {
							if (node.isOpen) {
								...
								keyboard
									.text(`📍 ${node.name}`, `modify:region:${node.uuid}`)
									.row();
							}
						}
						keyboard.text("🚫 Cancel 取消", "modify:cancel");
			...
			case "sessionType":
				...
				keyboard = new InlineKeyboard()
					.text("MP-BGP + ENH (推荐)", `modify:sessionType:mpbgp_enh`)
					.row()
					.text("MP-BGP Only", `modify:sessionType:mpbgp`)
					.row()
					.text("IPv6 + IPv4 独立会话", `modify:sessionType:separate`)
					.row()
			...
			case "mtu":
				...
				keyboard = new InlineKeyboard()
					...
					.text("1400", `modify:mtu:${uuid}:1400`)
					.row()
					.text("1380", `modify:mtu:${uuid}:1380`)
					.text("1360", `modify:mtu:${uuid}:1360`)
					.row()
					.text("🚫 Cancel 取消", "modify:cancel");
			...
			case "psk":
				...
				keyboard = new InlineKeyboard()
					.text("🔄 Generate New 生成新密钥", `modify:psk:${uuid}:generate`)
					.row()
					.text("❌ Disable PSK 禁用", `modify:psk:${uuid}:disable`)
					.row()
					.text("🚫 Cancel 取消", "modify:cancel");
	...
	bot.command("remove", async (ctx) => {
		...
		if (args[0] && isAsnInput(args[0])) {
			if (!isAdmin) {
				await ctx.reply(
					"❌ Only admin can remove other ASN peers\n只有管理员可以删除其他 ASN 的 Peer",
				);
				return;
			}
			targetAsn = normalizeAsn(args[0]);
		...
		try {
			...
			const sessions = (result.data?.sessions || []).filter(
				(s: { status: number }) => s.status !== 5,
			); // Exclude QUEUED_FOR_DELETE

			if (sessions.length === 0) {
				await ctx.reply(
					`ℹ️ AS${targetAsn} has no peers to remove.\nAS${targetAsn} 没有可删除的 Peer`,
				);
				return;
			}
			...
			sessions.forEach(
				(s: {
					...
					keyboard
						.text(`${s.routerName || s.router}`, `remove:select:${s.uuid}`)
						.row();
				},
			);
	...
	bot.command("restart", async (ctx) => {
		...
		if (args[0] && isAsnInput(args[0])) {
			if (!isAdmin) {
				await ctx.reply(
					"❌ Only admin can restart other ASN peers\n只有管理员可以重启其他 ASN 的 Peer",
				);
				return;
			}
			targetAsn = normalizeAsn(args[0]);
		...
		try {
			...
			const sessions = (result.data?.sessions || []).filter(
				(s: { status: number }) => s.status === PeeringStatus.ENABLED,
			);

			if (sessions.length === 0) {
				await ctx.reply(
					`❌ AS${targetAsn} has no active peers\nAS${targetAsn} 没有活跃的 Peer`,
				);
				return;
			}
			...
			if (sessions.length === 1) {
				...
				for (const s of sessions) {
					keyboard
						.text(s.router, `restart:${targetAsn}:${s.uuid}:${s.router}`)
						.row();
				}
				await ctx.reply(
					`🔄 *Restart Peer*\n重启 Peer\n\n` +
	...
	async function executeRestart(
		...
		await ctx.reply(
			`⏳ Restarting peer for AS${asn} @ ${router}...\n正在重启...`,
		);
	...
	bot.command("status", async (ctx) => {
		...
		try {
			...
			const fetchPromises = sessions.map(
				async (session: {
					router: string;
					routerName?: string;
					ipv6?: string;
	...
	bot.command("peers", async (ctx) => {
		...
		if (args[0] && isAsnInput(args[0])) {
			if (!isAdmin) {
				await ctx.reply(
					"❌ Only admin can list other ASN peers\n只有管理员可以查看其他 ASN 的 Peer",
				);
				return;
			}
			targetAsn = normalizeAsn(args[0]);
