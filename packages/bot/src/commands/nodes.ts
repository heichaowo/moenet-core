...
const CONTINENTS: Record<number, string> = { 1: 'AS', 2: 'NA', 3: 'EU', 4: 'OC', 5: 'OC' };
...
async function fetchNodes(): Promise<RouterRow[]> {
    ...
    return (result.data.routers as RouterRow[]).slice().sort((a, b) => a.name.localeCompare(b.name));
}
...
export function registerNodeCommands(bot: Bot<BotContext>) {
    ...
    bot.callbackQuery(/^node:v:(.+)$/, async (ctx) => {
        await ctx.answerCallbackQuery();
        await showNodeDetail(ctx, ctx.match[1]!, ctx.callbackQuery.message?.message_id);
    });
    ...
    bot.callbackQuery(/^node:e:(.+)$/, async (ctx) => {
        if (!isAdmin(ctx)) { await ctx.answerCallbackQuery('❌ Admin only'); return; }
        await ctx.answerCallbackQuery();
        await showEditMenu(ctx, ctx.match[1]!, ctx.callbackQuery.message?.message_id);
    });

    // Toggle a boolean field.
    bot.callbackQuery(/^node:t:(.+):(allowCnPeers|supportsIpv4|supportsIpv6)$/, async (ctx) => {
        if (!isAdmin(ctx)) { await ctx.answerCallbackQuery('❌ Admin only'); return; }
        const name = ctx.match[1]!;
        const field = ctx.match[2]! as 'allowCnPeers' | 'supportsIpv4' | 'supportsIpv6';

        const nodes = await fetchNodes();
        const node = nodes.find((n) => n.name === name);
        if (!node) { await ctx.answerCallbackQuery('❌ Not found'); return; }

        const next = !(node[field] ?? true);
        const res = await apiRequest('/admin', 'POST', {
            action: 'updateRouter', name, updates: { [field]: next },
        }, config.apiToken);
        if (res.code !== 0) { await ctx.answerCallbackQuery(`❌ ${res.message}`); return; }
    ...
    bot.callbackQuery(/^node:et:(.+):(location|provider|maxPeers)$/, async (ctx) => {
        if (!isAdmin(ctx)) { await ctx.answerCallbackQuery('❌ Admin only'); return; }
        const name = ctx.match[1]!;
    ...
    bot.callbackQuery(/^node:p:(.+)$/, async (ctx) => {
        if (!isAdmin(ctx)) { await ctx.answerCallbackQuery('❌ Admin only'); return; }
        await ctx.answerCallbackQuery();
        await showNodePeers(ctx, ctx.match[1]!, ctx.callbackQuery.message?.message_id);
    });
    ...
    bot.callbackQuery(/^node:b:(.+)$/, async (ctx) => {
        if (!isAdmin(ctx)) { await ctx.answerCallbackQuery('❌ Admin only'); return; }
        await ctx.answerCallbackQuery();
        await showBootstrap(ctx, ctx.match[1]!, false, ctx.callbackQuery.message?.message_id);
    });
    bot.callbackQuery(/^node:br:(.+)$/, async (ctx) => {
        if (!isAdmin(ctx)) { await ctx.answerCallbackQuery('❌ Admin only'); return; }
        await ctx.answerCallbackQuery('🔄 Regenerating...');
        await showBootstrap(ctx, ctx.match[1]!, true, ctx.callbackQuery.message?.message_id);
    });
    ...
    bot.callbackQuery(/^node:dc:(.+)$/, async (ctx) => {
        ...
        const name = ctx.match[1]!;
        const res = await apiRequest('/admin', 'POST', { action: 'deleteRouter', name }, config.apiToken);
        if (res.code !== 0) {
            await ctx.editMessageText(`❌ Failed to delete ${code(name)}: ${res.message}`, { parse_mode: 'Markdown' });
            return;
        }
        const n = (res.data as { sessionsDeleted?: number })?.sessionsDeleted ?? 0;
        await ctx.editMessageText(
            `✅ Node ${code(name)} deleted (${n} session(s) removed).`,
            { parse_mode: 'Markdown', reply_markup: new InlineKeyboard().text('🔙 Nodes', 'node:list') },
        );
    });
    ...
    bot.on('message:text', async (ctx, next) => {
        ...
        if (edit) {
            ...
            const res = await apiRequest('/admin', 'POST', {
                action: 'updateRouter', name: edit.name, updates: { [edit.field]: value },
            }, config.apiToken);
        ...
        switch (wizard.step) {
            case 'name':
                ...
                await ctx.reply('Step 2/11: Enter hostname (e.g., `lax1.edge.moenet.work`):\n请输入主机名:', { parse_mode: 'Markdown' });
                break;
            case 'hostname':
                wizard.data.hostname = text;
                wizard.step = 'ipv4';
                await ctx.reply('Step 3/11: Enter public IPv4 (or `skip` if no IPv4):\n请输入公网 IPv4 (或输入 `skip` 跳过):', { parse_mode: 'Markdown' });
                break;
            case 'ipv4':
                wizard.data.ipv4 = text.toLowerCase() === 'skip' ? null : text;
                wizard.step = 'ipv6';
                await ctx.reply('Step 4/11: Enter public IPv6 (or `skip` if no IPv6):\n请输入公网 IPv6 (或输入 `skip` 跳过):', { parse_mode: 'Markdown' });
                break;
            case 'ipv6':
                ...
                wizard.step = 'role';
                await ctx.reply('Step 5/11: Enter role (`rr` or `client`):\n请输入角色 (`rr` 或 `client`):', { parse_mode: 'Markdown' });
                break;
            case 'role':
                ...
                wizard.step = 'region';
                await ctx.reply('Step 6/11: Enter region (e.g., `US`, `EU`, `AP`):\n请输入区域:', { parse_mode: 'Markdown' });
                break;
            case 'region':
                wizard.data.region = text;
                wizard.step = 'location';
                await ctx.reply('Step 7/11: Enter location (e.g., `Los Angeles`):\n请输入位置:', { parse_mode: 'Markdown' });
                break;
            case 'location':
                wizard.data.location = text;
                wizard.step = 'provider';
                await ctx.reply('Step 8/11: Enter provider (e.g., `RackNerd`, `BuyVM`):\n请输入提供商:', { parse_mode: 'Markdown' });
                break;
            case 'provider':
                wizard.data.provider = text;
                wizard.step = 'bandwidth';
                await ctx.reply('Step 9/11: Enter bandwidth (e.g., `1G`, `10G`):\n请输入带宽:', { parse_mode: 'Markdown' });
                break;
            case 'bandwidth':
                wizard.data.bandwidth = text;
                wizard.step = 'max_peers';
                await ctx.reply('Step 10/11: Enter max peers (e.g., `50`):\n请输入最大 Peer 数:', { parse_mode: 'Markdown' });
                break;
            case 'max_peers': {
                ...
                await ctx.reply('Step 11/11: Allow China peers? (`yes` or `no`):\n是否允许中国大陆 Peer? (`yes` 或 `no`):', { parse_mode: 'Markdown' });
                break;
            }
            case 'allow_cn': {
...
async function showNodeList(ctx: BotContext, editMessageId?: number) {
    ...
    if (nodes.length === 0) {
        ...
        if (editMessageId) await ctx.api.editMessageText(ctx.chat!.id, editMessageId, msg);
    ...
    const keyboard = new InlineKeyboard();
    nodes.forEach((n, i) => {
        const cap = n.maxPeers ? `${n.sessionCount ?? 0}/${n.maxPeers}` : `${n.sessionCount ?? 0}`;
    ...
    if (editMessageId) {
        try {
            await ctx.api.editMessageText(ctx.chat!.id, editMessageId, text, { parse_mode: 'Markdown', reply_markup: keyboard });
        } catch (e) {
...
async function showNodeDetail(ctx: BotContext, name: string, editMessageId?: number) {
    const nodes = await fetchNodes();
    ...
    const keyboard = new InlineKeyboard().text('🤝 Peer here', `peer:here:${n.name}`).row();
    if (admin) {
        keyboard
...
async function showEditMenu(ctx: BotContext, name: string, editMessageId?: number) {
    const nodes = await fetchNodes();
    const n = nodes.find((r) => r.name === name);
    if (!n) { await ctx.reply('❌ Node not found.'); return; }
    ...
    const keyboard = new InlineKeyboard()
        .text(`🇨🇳 Allow CN: ${onoff(n.allowCnPeers)}`, `node:t:${name}:allowCnPeers`)
        .row()
...
async function showNodePeers(ctx: BotContext, name: string, editMessageId?: number) {
    const res = await apiRequest('/admin', 'POST', { action: 'enumSessions' }, config.apiToken);
    const sessions = (Array.isArray(res.data?.sessions) ? res.data.sessions : []) as Array<{
        asn: number; status: number; routerName?: string;
    }>;
...
async function showBootstrap(ctx: BotContext, name: string, refresh: boolean, editMessageId?: number) {
    let token: string | undefined;

    if (refresh) {
        const res = await apiRequest('/admin', 'POST', { action: 'regenerateBootstrapToken', name }, config.apiToken);
        if (res.code !== 0) { await ctx.reply(`❌ ${res.message}`); return; }
        token = (res.data as { token?: string })?.token;
    } else {
        const res = await apiRequest('/admin', 'POST', { action: 'getRouter', name }, config.apiToken);
        token = (res.data as { router?: { bootstrapToken?: string } })?.router?.bootstrapToken ?? undefined;
        if (!token) {
            const gen = await apiRequest('/admin', 'POST', { action: 'regenerateBootstrapToken', name }, config.apiToken);
...
async function createNode(ctx: BotContext, data: NodeData) {
    try {
        ...
        const nodeId = (result.data as { router?: { nodeId?: number } })?.router?.nodeId ?? 'N/A';
        const coreUrl = config.coreUrl || 'https://api.moenet.work';
