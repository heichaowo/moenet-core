...
import { getAsnInfo, getNetworkStats, getRanking } from "../services/iedonApi";
...
function formatRankPage(ranking: RankingEntry[], page: number): string {
	...
	for (const item of items) {
		...
		lines.push(
			`${medal}\`${String(item.rank).padStart(3)}\` AS${item.asn} | ${name} | ${item.index}${highlight}`,
		);
	}
	...
	bot.command("stats", async (ctx) => {
		...
		try {
			...
			if (local) {
				...
				const routers = (routerResult.data?.routers ?? []) as Array<{
					name: string;
					location: string;
					sessionCount: number;
					isOpen: boolean;
				}>;
				const totalPeers = routers.reduce(
					(sum: number, r) => sum + r.sessionCount,
					0,
				);
