 ...
 */
export async function validateIpOwnership(
	asn: number,
	ip: string,
	...
	try {
		...
		const response = await fetch(
			`${BURBLE_API}/${objectType}/${encodeURIComponent(ip)}`,
		);
		...
		const mntBy = obj.Attributes.filter(
			(a: [string, string]) => a[0] === "mnt-by",
		).map((a: [string, string]) => a[1]);

		// Check if ASN's mntner matches
		const asnMntner = `MNT-${asn}`;
		const asnMntnerAlt = `AS${asn}-MNT`;

		const ownerMatch = mntBy.some(
			(m: string) =>
		...
		const routeResponse = await fetch(
			`${BURBLE_API}/${routeType}/${encodeURIComponent(ip)}`,
		);

		if (routeResponse.ok) {
			const routeData = (await routeResponse.json()) as Record<
				string,
				RegistryObject
			>;
			...
			if (routeKeys.length > 0) {
				...
				if (routeObj?.Attributes) {
					const origin = routeObj.Attributes.find(
						(a: [string, string]) => a[0] === "origin",
					)?.[1];
		...
		return {
			valid: false,
			warning:
				`⚠️ IP ownership verification failed. Expected AS${asn}, found: ${mntBy.join(", ") || "unknown"}\n` +
